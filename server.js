...
if(!APIKEY) throw new Error("Missing XKIRO_API_KEY");
fs.mkdirSync(WORKDIR,{recursive:true});

const app=express();
app.use(express.json({limit:"20mb"}));
app.get('/',(_,r)=>r.json({ok:true,service:"claude-code-telegram",model:PROVIDER_MODEL,claudeModel:MODEL,provider:"xkiro",streaming:true}));
app.get('/health',(_,r)=>r.json({ok:true,model:PROVIDER_MODEL,claudeModel:MODEL,provider:"xkiro",streaming:true}));
...
function streamAnthropic(r,m){
  r.setHeader('content-type','text/event-stream');r.setHeader('cache-control','no-cache, no-transform');r.setHeader('connection','keep-alive');
  sse(r,{type:'message_start',message:{...m,content:[],stop_reason:null,stop_sequence:null}});let i=0;
  for(const b of m.content||[]){sse(r,{type:'content_block_start',index:i,content_block:b.type==='tool_use'?{type:'tool_use',id:b.id,name:b.name,input:{}}:{type:'text',text:''}});if(b.type==='text')sse(r,{type:'content_block_delta',index:i,delta:{type:'text_delta',text:b.text}});else sse(r,{type:'content_block_delta',index:i,delta:{type:'input_json_delta',partial_json:JSON.stringify(b.input||{})}});sse(r,{type:'content_block_stop',index:i++})}
  sse(r,{type:'message_delta',delta:{stop_reason:m.stop_reason,stop_sequence:null},usage:{output_tokens:m.usage.output_tokens}});sse(r,{type:'message_stop'});r.end();
}

// xKiro cuts blocking requests at 95s. Always stream upstream, then aggregate the SSE
// into the Anthropic response Claude Code expects. This removes the long-tool-turn cutoff.
async function readXKiroStream(response){
  if(!response.body)throw new Error('xKiro returned no response body');
  const reader=response.body.getReader(),decoder=new TextDecoder();let buffer='',id='',model=PROVIDER_MODEL,content='',finish=null;const toolMap=new Map();let usage={};
  const consume=raw=>{for(const line of raw.split(/\r?\n/)){if(!line.startsWith('data:'))continue;const payload=line.slice(5).trim();if(!payload||payload==='[DONE]')continue;let d;try{d=JSON.parse(payload)}catch{continue}id=d.id||id;model=d.model||model;const choice=d.choices?.[0],delta=choice?.delta||{};if(delta.content)content+=delta.content;for(const tc of delta.tool_calls||[]){const idx=tc.index??0,cur=toolMap.get(idx)||{id:'',name:'',arguments:''};if(tc.id)cur.id=tc.id;if(tc.function?.name)cur.name+=tc.function.name;if(tc.function?.arguments)cur.arguments+=tc.function.arguments;toolMap.set(idx,cur)}if(choice?.finish_reason)finish=choice.finish_reason;if(d.usage)usage=d.usage;}};
  while(true){const {value,done}=await reader.read();if(done)break;buffer+=decoder.decode(value,{stream:true});const parts=buffer.split(/\r?\n\r?\n/);buffer=parts.pop()||'';for(const p of parts)consume(p)}
  if(buffer)consume(buffer);
  return {id,model,choices:[{message:{role:'assistant',content,tool_calls:[...toolMap.values()].map(x=>({id:x.id||`tool_${crypto.randomUUID()}`,type:'function',function:{name:x.name||'tool',arguments:x.arguments||'{}'}}))},finish_reason:finish}],usage};
}

app.post('/v1/messages',async(req,res)=>{
  if(!authOK(req))return res.status(401).json({type:'error',error:{type:'authentication_error',message:'Unauthorized'}});
  try{
    const b=req.body||{},q={model:PROVIDER_MODEL,messages:toOpenAI(b),max_tokens:Number(b.max_tokens||8192),stream:true};const ts=tools(b.tools);if(ts?.length)q.tools=ts;
    if(b.tool_choice?.type==='auto')q.tool_choice='auto';if(b.tool_choice?.type==='any')q.tool_choice='required';if(b.tool_choice?.type==='tool')q.tool_choice={type:'function',function:{name:b.tool_choice.name}};
    if(b.output_config?.effort)q.reasoning_effort=b.output_config.effort;
    console.log(`[bridge] claudeModel=${b.model||MODEL} providerModel=${q.model} messages=${q.messages.length} tools=${q.tools?.length||0} stream=true`);
    const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),TIMEOUT);let u,d;
    try{
      u=await fetch(`${BASE}/chat/completions`,{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${APIKEY}`},body:JSON.stringify(q),signal:controller.signal});
      if(!u.ok){const raw=await u.text();console.error(`[bridge] xKiro ${u.status}: ${raw.slice(0,4000)}`);return res.status(u.status).json({type:'error',error:{type:u.status===429?'rate_limit_error':'api_error',message:`xKiro ${u.status}: ${raw.slice(0,4000)}`}})}
      d=await readXKiroStream(u);
    }finally{clearTimeout(timer)}
    const m=anthropic(d,b.model||MODEL);if(b.stream)return streamAnthropic(res,m);return res.json(m);
  }catch(e){const msg=e?.name==='AbortError'?'xKiro request timed out.':String(e?.message||e);console.error(`[bridge] error: ${e.stack||e}`);if(!res.headersSent)res.status(500).json({type:'error',error:{type:'api_error',message:msg}});else res.end()}
});

const active=new Map();const allowed=m=>!ALLOWED.size||ALLOWED.has(String(m.chat.id))||ALLOWED.has(String(m.from?.id));const clip=(s,n=3900)=>String(s||'').length<=n?String(s||''):String(s||'').slice(0,n)+'\n…[truncated]';
function runClaude(prompt,chat){return new Promise((resolve,reject)=>{
  ...
  const args=['-p',prompt,'--output-format','text','--permission-mode','bypassPermissions','--no-session-persistence','--verbose'];console.log(`[claude] command=claude ${args.map((x,i)=>i===1?'[prompt]':x).join(' ')}`);
  const p=spawn('claude',args,{cwd:WORKDIR,env,stdio:['ignore','pipe','pipe']});let out='',err='';const started=Date.now();active.set(chat,{child:p,started,stderr:''});const to=setTimeout(()=>{console.error(`[claude] timeout chat=${chat}`);p.kill('SIGTERM')},TIMEOUT);
  p.stdout.on('data',b=>{out+=b.toString()});p.stderr.on('data',b=>{const s=b.toString();err+=s;console.error(`[claude] ${s.trimEnd()}`);const a=active.get(chat);if(a)a.stderr=err.trim().split(/\r?\n/).at(-1)||''});
  ...
  p.on('close',(code,signal)=>{clearTimeout(to);active.delete(chat);console.log(`[claude] exited code=${code} signal=${signal} stdout=${out.length} stderr=${err.length} elapsed=${Date.now()-started}ms`);if(code===0)resolve(out.trim());else reject(new Error(`Claude Code exited code=${code} signal=${signal}\n${err.slice(-3000)||'No stderr output. Check xKiro/model configuration and Claude Code startup.'}`))})
})}

let bot;
function startTelegram(){
  bot=new TelegramBot(TG,{polling:{autoStart:false,params:{timeout:25}}});
  bot.on('polling_error',e=>{console.error('Telegram:',e.message);if(String(e?.message||'').includes('409 Conflict')){bot.stopPolling().catch(()=>{});setTimeout(()=>bot.startPolling().catch(err=>console.error('Telegram restart:',err.message)),7000)}});
  ...
  bot.startPolling().catch(e=>console.error('Telegram start:',e.message));
}
startTelegram();
app.listen(PORT,'0.0.0.0',()=>console.log(`Listening ${PORT}; Claude model=${MODEL}; provider model=${PROVIDER_MODEL}; xKiro=${BASE}; upstream=stream`));
